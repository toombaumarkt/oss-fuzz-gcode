...
PointSegment *GcodeParser::processCommand(const QStringList &args)
{
    ...
    if (!qIsNaN(speed)) {
        #if DEBUG_OUTPUT
            debug_message += " F: " + std::to_string(speed);
        #endif
    ...
    if (!qIsNaN(spindleSpeed)) {
        #if DEBUG_OUTPUT
            debug_message += " S: " + std::to_string(spindleSpeed);
        #endif
    ...
    if (!qIsNaN(dwell)){
        #if DEBUG_OUTPUT
            debug_message += " P: " + std::to_string(dwell);
        #endif
    ...
    foreach (float code, gCodes) {
        #if DEBUG_OUTPUT
            debug_message = "G: " + std::to_string(code) + debug_message;
        #endif
