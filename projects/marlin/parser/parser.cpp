...
void GCodeParser::parse(char *p) {
  ...
  while (const char param = uppercase(*p++)) {  // Get the next parameter. A NUL ends the loop
    ...
    if (PARAM_OK(param)) {
      ...
      #if ENABLED(DEBUG_GCODE_PARSER)
        if (debug) {
          std::cout << "Got param " << (param) << " at index " << p - command_ptr - 1;
        }
      #endif
      ...
      if (!has_val && !string_arg) {            // No value? First time, keep as string_arg
        string_arg = p - 1;
        #if ENABLED(DEBUG_GCODE_PARSER)
          if (debug) std::cout << " string_arg: " << ((void*)string_arg); // DEBUG
        #endif
      }

      if (TERN0(DEBUG_GCODE_PARSER, debug)) std::cout << "\n";

      //std::cout << param << valptr[0];
...
void GCodeParser::unknown_command_warning() {
  ...
  void GCodeParser::debug() {
    std::cout << "Command: ", command_ptr, " (", command_letter;
    std::cout << codenum;
    std::cout << (")");
    #if ENABLED(FASTER_GCODE_PARSER)
      std::cout << " args: { ";
      for (char c = 'A'; c <= 'Z'; ++c) if (seen(c)) std::cout << c;
      std::cout << ('}');
    #else
      std::cout << " args: { ", command_args, " }");
    #endif
    if (string_arg) {
      std::cout << " string: \"", string_arg;
      std::cout << ('"');
    }
    std::cout <<  ("\n");
    for (char c = 'A'; c <= 'Z'; ++c) {
      if (seen(c)) {
        std::cout << "Code '", c; std::cout << "':";
        if (has_value()) {
          std::cout <<  (
            "\n    float: ", value_float(),
            "\n     long: ", value_long(),
