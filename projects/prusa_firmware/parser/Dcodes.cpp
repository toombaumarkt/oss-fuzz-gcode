...
void print_mem(daddr_t address, daddr_t count, dcode_mem_t type, uint8_t countperline = 16)
{
	...
	while (count)
	{
		...
		while (count && count_line)
		{
			...
			switch (type)
			{
			case dcode_mem_t::sram: break;//data = *((uint8_t*)address); break;
			case dcode_mem_t::eeprom: break;//data = eeprom_read_byte((uint8_t*)address); break;
			case dcode_mem_t::progmem: break;
#if defined(DEBUG_DCODE6) || defined(DEBUG_DCODES) || defined(XFLASH_DUMP)
            case dcode_mem_t::xflash: break;//xflash_rd_data(address, &data, 1); break;
#else
...
void write_mem(uint16_t address, uint16_t count, const uint8_t* data, const dcode_mem_t type)
{
    for (uint16_t i = 0; i < count; i++)
    {
        switch (type)
        {
        case dcode_mem_t::sram: break;//*((uint8_t*)address) = data[i]; break;
        case dcode_mem_t::eeprom: break;//eeprom_write_byte((uint8_t*)address, data[i]); break;
        case dcode_mem_t::progmem: break;
        case dcode_mem_t::xflash: break;
        }
        ++address;
