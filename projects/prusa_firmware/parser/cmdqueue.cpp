...
{
    size_t len = from_progmem ? strlen(cmd) : strlen(cmd);
    ...
    if (cmdqueue_could_enqueue_back(len)) {
        ...
        ////SERIAL_ECHORPGM(MSG_Enqueing);
        if (from_progmem)
            int a;////SERIAL_PROTOCOLRPGM(cmd);
        else
            int a;////SERIAL_ECHO(cmd);
...
void get_command(const char * line, size_t length)
{
  ...
  while (index < length /*&& !saved_printing) *//*|| (MYSERIAL.available() > 0 && isPrintPaused)) */&& !cmdqueue_serial_disabled) {  //is print is saved (crash detection or filament detection), dont process data from serial line
	
    char serial_char = line[index++];
       ...
       serial_count >= (MAX_CMD_SIZE - 1) )
    {
      ...
      if(!comment_mode){
		  ...
		  if ((strstr(cmdbuffer+bufindw+CMDHDRSIZE, "PRUSA") == NULL) &&
			  ...
			  if(gcode_N != gcode_LastN+1 && (strstr(cmdbuffer+bufindw+CMDHDRSIZE, "M110") == NULL) ) {
      ...
      if (index >= length || ! cmdqueue_could_enqueue_back(MAX_CMD_SIZE-1, true))
          return;
    ...
    if(farm_mode && (serial_count > 0)){
        if (true /* deactivated timer -- farm_incomplete_command_timeout_timer.expired(800)*/) {
